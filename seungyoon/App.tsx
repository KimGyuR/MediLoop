...
function Header({ compact = false, onBellPress }: { compact?: boolean; onBellPress: () => void }) {
  return (
    <View style={[styles.header, compact && styles.headerCompact]}>
      <Text style={[styles.time, compact && styles.timeSmall]}>9:41</Text>
      <Text style={[styles.logo, compact && styles.logoSmall]}>Mediloop</Text>
      <Text style={styles.signal}>LTE</Text>
...
function Label({ icon, text: value }: { icon: string; text: string }) {
  return (
                  ...
                  <View key={item.id} style={styles.fillBagReservationCard}>
                    ...
                    {expanded && (
                      <View style={styles.fillBagExpandedContent}>
                        ...
                        <View style={styles.fillBagBundleSectionCard}>
                          <View style={styles.row}>
                            <Text style={styles.bellGreen}>♧</Text>
...
function ProfileScreen4({
  ...
  const visibleSymptoms = showAllSymptoms ? symptomItems : symptomItems.slice(0, 3);
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
      ...
      <View style={styles.profileMenuCard}>
        ...
        <TouchableOpacity style={styles.profileMenuRow} onPress={onLogout}>
          <Text style={styles.profileMenuLogoutIcon}>↪</Text>
...
function BottomNav({ active, onChange }: { active: Tab; onChange: (tab: Tab) => void }) {
  const items: { id: Tab; label: string; icon: string }[] = [
    ...
    { id: 'fillbag', label: 'Fill Bag', icon: '◌' },
...
const styles = StyleSheet.create({
  ...
  logo: { position: 'absolute', left: 52, top: 44, fontSize: 18, fontWeight: '800', color: '#030706' },
  ...
  badge: { position: 'absolute', right: -1, top: -3, width: 12, height: 12, borderRadius: 6, backgroundColor: '#E96F73' },
  bellGlyph: {
    width: 18,
    height: 18,
    position: 'relative',
